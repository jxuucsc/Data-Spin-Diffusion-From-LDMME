...
for line in open('totalE.out'):
  ...
  if "nBands" in line:
    nElectrons = int(float(re.findall(r"[+-]?\d+\.\d*", line)[0]))
    nValence = nElectrons
    nBands = int(re.findall(r"[+-]?\d+\.\d*|[+-]?\d+", line)[1])
    nKpoints = int(re.findall(r"[+-]?\d+\.\d*|[+-]?\d+", line)[2])
  if "spintype spin-orbit" in line or "spintype vector-spin" in line:
    SOC = True
    mlwfType = (np.complex128 if SOC else np.float64)
if not SOC:
  nValence = nElectrons // 2 #number of valence bands
if mu == np.nan:
...
cellMap = cellMapIn[:,:3].astype(np.int32)
...
Hreduced = np.fromfile("wannier.mlwfH"+suffix, dtype=mlwfType).reshape((kfoldProd,nBands,nBands)).swapaxes(1,2)
