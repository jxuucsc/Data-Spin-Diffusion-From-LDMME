...
excess_density = 1e14 # excess carrier density (elec. - hole) in cm^-d
mu = 0.8 / units.eV # chemical potential or Fermi level, from eV to au
...
# real-time dynamics
print("\ndo real-time dynamics")
t = np.linspace(0,7*tau[1],3000,dtype=np.float64)
...
np.savetxt("analysis_ldmme/spin_relevance.dat", np.transpose([spin_rel[1:], tau[1:]*units.ps]))
...
ldiff_srel = np.where(prod_srel_ldiff < 1e-3*np.max(prod_srel_ldiff), 0, np.abs(ldiff))
xmax = np.max(ldiff_srel)
print("\ndo real-space simulations")
...
_,_,_ = tlRR.solve_tl_fromldmme_RayleighRitz_sevp("lambda inv", -eph.Lpinv_Lv, UL_l_RR, UR_l_RR, np.array([0,3],np.int32), "sx vsx, with Left, sevp", True, "smallest inv real abs")
_,_,_ = tlRR.solve_tl_fromldmme_RayleighRitz_sevp("lambda inv", -eph.Lpinv_Lv, UL_l_RR, UR_l_RR, np.array([1,4],np.int32), "sy vsy, with Left, sevp", True, "smallest inv real abs")
_,_,_ = tlRR.solve_tl_fromldmme_RayleighRitz_sevp("lambda inv", -eph.Lpinv_Lv, UL_l_RR, UR_l_RR, np.array([2,5],np.int32), "sz vsz, with Left, sevp", True, "smallest inv real abs")
...
lambda_cDD,UL_cDD,UR_cDD = tlRR.solve_tl_fromldmme_RayleighRitz_sevp("lambda inv", -eph.Lpinv_Lv, UL_l_RR, UR_l_RR, np.array([0,1,2,3,4,5],np.int32), "s vs, with Left, sevp", True, "smallest inv real abs")
