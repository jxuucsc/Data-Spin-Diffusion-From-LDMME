...
from approximate_lifetimess_diffusion_lengths_gan import *
...
# rotate U for later analysis for grbn and gan
UL_rotate,UR_rotate = get_U_rotate_within_deg(UL, UR, ob_scale, 1, 3)
e_rotate = np.array([(e[1]+e[2])/2., (e[1]+e[2])/2.], np.complex128)
Lcorr = np.einsum("am,bm->ab", np.einsum("am,m->am", UR_rotate, -e_rotate, optimize=True), UL_rotate.conj(), optimize=True) \
       - np.einsum("am,bm->ab", np.einsum("am,m->am", UR[:,1:3], -e[1:3], optimize=True), UL[:,1:3].conj(), optimize=True)
eph.L = eph.L + Lcorr
UL[:,1:3] = UL_rotate
UR[:,1:3] = UR_rotate
...
x = np.linspace(0,10*xmax,5000,dtype=np.float64)
...
approximate_diffusion_lengths_gan(e, UL, UR, UL_l_RR, UR_l_RR, eph, elec, orthnorm_URR, x, rhopert, ob)

t5_1 = timer()
print("time for Rayleigh-Ritz method: ",t5_1-t4)

# more diffusion length analysis
approximate_diffusion_lengths_gan_more(e, UL, UR, eph, elec, orthnorm_URR)

t5_2 = timer()
print("time for more diffusion length analysis for gan: ",t5_2-t5_1)
#'''
