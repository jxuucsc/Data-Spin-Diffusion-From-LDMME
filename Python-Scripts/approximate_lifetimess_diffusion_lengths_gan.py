...
def approximate_lifetimes_gan(UL_RR, UR_RR, eph, elec):
  print("\n##################################################")
  print("lifetime analysis for gan:")
  print("##################################################")
  orig_stdout = sys.stdout
  f = open('approximate_lifetimes_gan.out', 'w')
  sys.stdout = f
...
def approximate_diffusion_lengths_gan_aux(e, UL, UR, UL_RR, UR_RR, eph, elec):
...
def approximate_diffusion_lengths_gan_more(e, UL, UR, eph, elec, orthnorm_URR):
  print("\n##################################################")
  print("more diffusion length analysis for gr-bn:")
  print("##################################################")
  orig_stdout = sys.stdout
  ftmp = open('approximate_diffusion_lengths_gan_more.out', 'w')
  sys.stdout = ftmp
  
  UL_l_RR, UR_l_RR = setU_RayleighRitz_forLength(orthnorm_URR, UL, UR, eph.L, elec.Lv, 3, elec.nmode, False)
  approximate_diffusion_lengths_gan_aux(e[1:], UL[:,1:], UR[:,1:], UL_l_RR, UR_l_RR, eph, elec)
...
def approximate_diffusion_lengths_gan(e, UL, UR, UL_l_RR, UR_l_RR, eph, elec, orthnorm_URR, x, rhopert, ob):
  print("\n##################################################")
  print("diffusion length analysis for gan based on RR method:")
  print("##################################################")
  orig_stdout = sys.stdout
  ftmp = open('approximate_diffusion_lengths_gan_RR.out', 'w')
  sys.stdout = ftmp
  ...
  tlRR.solve_tl_fromldmme_RayleighRitz("lambda", -eph.L, elec.Lv, UL_l_RR, UR_l_RR, np.array([0,1,2,3,4,5],np.int32), "s vs, with Left, lambda", True, "smallest real abs")
  ...
  print("--------------------------------------------------")
  lambda_cDD_sevp,UL_cDD_sevp,UR_cDD_sevp = tlRR.solve_tl_fromldmme_RayleighRitz_sevp("lambda inv", -eph.Lpinv_Lv, UL_l_RR, UR_l_RR, np.array([0,1,2,3,4,5],np.int32), "s vs, with Left", True, "smallest inv real abs")
  ...
  Ox = diffuse_realspace_1d(x, True, rhopert[1], ob[1], UL_cDD_sevp, UR_cDD_sevp, lambda_cDD_sevp, "analysis_ldmme/Sxx_sxpert_cDD_sevp", 0)
  fit_Ox_expcos(x, Ox, "Sxx_sxpert_cDD_sevp")
  Ox = diffuse_realspace_1d(x, True, rhopert[2], ob[2], UL_cDD_sevp, UR_cDD_sevp, lambda_cDD_sevp, "analysis_ldmme/Syx_sypert_cDD_sevp", 0)
  fit_Ox_expcos(x, Ox, "Syx_sypert_cDD_sevp")
  Ox = diffuse_realspace_1d(x, True, rhopert[3], ob[3], UL_cDD_sevp, UR_cDD_sevp, lambda_cDD_sevp, "analysis_ldmme/Szx_szpert_cDD_sevp", 0)
  fit_Ox_expcos(x, Ox, "Szx_szpert_cDD_sevp")
  ...
  print("--------------------------------------------------")
  lambda_mcDD_sevp,UL_mcDD_sevp,UR_mcDD_sevp = tlRR.solve_tl_fromldmme_RayleighRitz_sevp("lambda inv", -eph.Lpinv_Lv, UL_l_RR_2, UR_l_RR_2, np.array([0,1,2,6,7,8],np.int32), "sr vsr, sl, vsr", True, "smallest inv real abs")
  
  Ox = diffuse_realspace_1d(x, True, rhopert[1], ob[1], UL_mcDD_sevp, UR_mcDD_sevp, lambda_mcDD_sevp, "analysis_ldmme/Sxx_sxpert_mcDD_sevp", 0)
  fit_Ox_expcos(x, Ox, "Sxx_sxpert_mcDD_sevp")
  Ox = diffuse_realspace_1d(x, True, rhopert[2], ob[2], UL_mcDD_sevp, UR_mcDD_sevp, lambda_mcDD_sevp, "analysis_ldmme/Syx_sypert_mcDD_sevp", 0)
  fit_Ox_expcos(x, Ox, "Syx_sypert_mcDD_sevp")
  Ox = diffuse_realspace_1d(x, True, rhopert[3], ob[3], UL_mcDD_sevp, UR_mcDD_sevp, lambda_mcDD_sevp, "analysis_ldmme/Szx_szpert_mcDD_sevp", 0)
  fit_Ox_expcos(x, Ox, "Szx_szpert_mcDD_sevp")
  
  print("\n--------------------------------------------------")
  print("real-space simulations using m-ss-DD model and GEVP (with sr vsr, sl vsr)")
  print("--------------------------------------------------")
  lambda_mcDD,UL_mcDD,UR_mcDD = tlRR.solve_tl_fromldmme_RayleighRitz_solutions_g2s("lambda inv", elec.Lv, -eph.L, UL_l_RR_2, UR_l_RR_2, np.array([0,1,2,6,7,8],np.int32), "sr vsr, sl vsr", True, "smallest inv real abs")
  
  Ox = diffuse_realspace_1d(x, True, rhopert[1], ob[1], UL_mcDD, UR_mcDD, lambda_mcDD, "analysis_ldmme/Sxx_sxpert_mcDD", 0)
  fit_Ox_expcos(x, Ox, "Sxx_sxpert_mcDD")
  Ox = diffuse_realspace_1d(x, True, rhopert[2], ob[2], UL_mcDD, UR_mcDD, lambda_mcDD, "analysis_ldmme/Syx_sypert_mcDD", 0)
  fit_Ox_expcos(x, Ox, "Syx_sypert_mcDD")
  Ox = diffuse_realspace_1d(x, True, rhopert[3], ob[3], UL_mcDD, UR_mcDD, lambda_mcDD, "analysis_ldmme/Szx_szpert_mcDD", 0)
  fit_Ox_expcos(x, Ox, "Szx_szpert_mcDD")
  
  print("\n--------------------------------------------------")
  print("real-space simulations using ss-DD model with appr. Linv (with sr vsr, sl vsl)")
  print("--------------------------------------------------")
  
  lambda_cDD_appr_sevp,UL_cDD_appr_sevp,UR_cDD_appr_sevp = tlRR.solve_tl_fromldmme_RayleighRitz_sevp("lambda inv", -Linv_appr_Lv, UL_l_RR, UR_l_RR, np.array([0,1,2,3,4,5],np.int32), "s vs, with Left", True, "smallest inv real abs")
  
  Ox = diffuse_realspace_1d(x, True, rhopert[1], ob[1], UL_cDD_appr_sevp, UR_cDD_appr_sevp, lambda_cDD_appr_sevp, "analysis_ldmme/Sxx_sxpert_cDD_appr_sevp", 0)
  fit_Ox_expcos(x, Ox, "Sxx_sxpert_cDD_appr_sevp")
  Ox = diffuse_realspace_1d(x, True, rhopert[2], ob[2], UL_cDD_appr_sevp, UR_cDD_appr_sevp, lambda_cDD_appr_sevp, "analysis_ldmme/Syx_sypert_cDD_appr_sevp", 0)
  fit_Ox_expcos(x, Ox, "Syx_sypert_cDD_appr_sevp")
  Ox = diffuse_realspace_1d(x, True, rhopert[3], ob[3], UL_cDD_appr_sevp, UR_cDD_appr_sevp, lambda_cDD_appr_sevp, "analysis_ldmme/Szx_szpert_cDD_appr_sevp", 0)
  fit_Ox_expcos(x, Ox, "Szx_szpert_cDD_appr_sevp")
  
  print("\n--------------------------------------------------")
  print("real-space simulations using ss-DD model with appr. Linv (with sr vsr, sl vsr)")
  print("--------------------------------------------------")
  lambda_cDD_appr,UL_cDD_appr,UR_cDD_appr = tlRR.solve_tl_fromldmme_RayleighRitz_sevp("lambda inv", -Linv_appr_Lv, UL_l_RR_2, UR_l_RR_2, np.array([0,1,2,6,7,8],np.int32), "sr vsr, sl, vsr", True, "smallest inv real abs")
  
  Ox = diffuse_realspace_1d(x, True, rhopert[1], ob[1], UL_cDD_appr, UR_cDD_appr, lambda_cDD_appr, "analysis_ldmme/Sxx_sxpert_cDD_appr", 0)
  fit_Ox_expcos(x, Ox, "Sxx_sxpert_cDD_appr")
  Ox = diffuse_realspace_1d(x, True, rhopert[2], ob[2], UL_cDD_appr, UR_cDD_appr, lambda_cDD_appr, "analysis_ldmme/Syx_sypert_cDD_appr", 0)
  fit_Ox_expcos(x, Ox, "Syx_sypert_cDD_appr")
  Ox = diffuse_realspace_1d(x, True, rhopert[3], ob[3], UL_cDD_appr, UR_cDD_appr, lambda_cDD_appr, "analysis_ldmme/Szx_szpert_cDD_appr", 0)
  fit_Ox_expcos(x, Ox, "Szx_szpert_cDD_appr")
