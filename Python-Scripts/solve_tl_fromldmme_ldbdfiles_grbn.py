...
mu = 0.1 / units.eV # chemical potential or Fermi level, from eV to au
...
valley = np.array([K, -K])
ind_pert_sel = np.array([0,1,2,3,7,8,9,10],np.int32) #np.arange(0,4,dtype=np.int32)
ind_ob_sel = np.array([0,1,2,3,7,8,9,10],np.int32) #np.arange(0,4,dtype=np.int32)
...
approximate_diffusion_lengths_grbn(e, UL, UR, UL_l_RR, UR_l_RR, eph, elec, orthnorm_URR, x, rhopert, ob)

t5_1 = timer()
print("time for Rayleigh-Ritz method: ",t5_1-t4)

# more diffusion length analysis
approximate_diffusion_lengths_grbn_more(e, UL, UR, eph, elec, orthnorm_URR)

t5_2 = timer()
print("time for more diffusion length analysis for grbn: ",t5_2-t5_1)
#'''
